...
import (
	...
	"github.com/google/osv-scalibr/extractor/filesystem/language/javascript/internal"
...
func parseNpmLockDependencies(dependencies map[string]packagelockjson.Dependency, finder *internal.JSONLineFinder, parentPath string) map[string]packageDetails {
...
func parseNpmLockPackages(packages map[string]packagelockjson.Package, finder *internal.JSONLineFinder) map[string]packageDetails {
	details := npmPackageDetailsMap{}
...
func parseNpmLock(lockfile packagelockjson.LockFile, finder *internal.JSONLineFinder) map[string]packageDetails {
...
func (e Extractor) extractPkgLock(_ context.Context, input *filesystem.ScanInput) ([]*extractor.Package, error) {
	...
	finder := internal.NewJSONLineFinder(string(b))
